...
export function ExamplesBrowser() {
  ...
  const filtered = useMemo(() => {
    return localizedExamples.filter((example) => {
  ...
  }, [category, query, localizedExamples]);

  const selectedExample = selected ? localizedExamples.find((ex) => ex.id === selected) : null;

  return (
    <>
      <div className="examples-toolbar">
        <div className="example-tabs" role="tablist" aria-label={t("Examples")}>
          {categories.map((item) => (
            <button
              key={item}
              type="button"
              ...
              className={`example-tab ${category === item ? "active" : ""}`}
              onClick={() => setCategory(item)}
            >
              {t(item)}
              <span>{item === "All" ? examples.length : examples.filter((example) => example.category === item).length}</span>
            </button>
          ))}
        </div>
        <label className="example-search">
          <Search size={16} />
          <span className="sr-only">{t("Search examples")}</span>
          <input
            value={query}
            onChange={(event) => setQuery(event.target.value)}
            placeholder={t("Search by capability or stack")}
          />
          {query && (
      ...
      {filtered.length === 0 && (
        <div className="examples-empty">
          <h2>{t("No matching examples")}</h2>
          <p>{t("Try a different capability, industry, or technology.")}</p>
        </div>
      )}

      {selectedExample && (
        <ExamplePanel example={selectedExample} onClose={() => setSelected(null)} />
      )}
    </>
  );
}
...
function ExamplePanel({
  ...
  example: ReturnType<typeof getLocalizedExample>;
  onClose: () => void;
}) {
  const { t } = useLanguage();

  return (
    <div className="example-panel-backdrop" role="presentation" onClick={onClose}>
      <aside
        className="example-panel"
        role="dialog"
        aria-modal="true"
        aria-labelledby="example-panel-title"
      ...
      >
        ...
        <div className="example-panel-content">
          <span className="eyebrow">
            {t(example.category)} · {example.eyebrow}
          </span>
          <h2 id="example-panel-title">{example.title}</h2>
          <p>{example.description}</p>
          <div className="panel-metric">
            <strong>{example.metric}</strong>
            <span>{example.timeline} {t("delivery window")}</span>
          </div>
          <h3>{t("Core capabilities")}</h3>
          <ul>
            {example.capabilities.map((item) => (
              <li key={item}>
                <Check size={15} />
                {item}
              </li>
          ...
          <div className="panel-actions">
            <Link href="/appointment" className="button primary" onClick={onClose}>
