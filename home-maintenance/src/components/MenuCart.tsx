...
const categoryLabels: Record<string, { label: string; labelAr: string; color: string }> = {
  electrical: { label: 'Electrical', labelAr: 'الكهرباء', color: 'bg-amber-100 text-amber-800' },
  plumbing:   { label: 'Plumbing',   labelAr: 'السباكة',  color: 'bg-sky-100 text-sky-800' },
  inspection: { label: 'Inspections', labelAr: 'الفحص والصيانة', color: 'bg-teal-100 text-teal-800' },
};
...
export default function MenuCart({ products, customerName, customerPhone = '', customerType, customerId, catalogError }: MenuCartProps) {
  ...
  const inspection = products.filter(p => p.category === 'inspection');
  ...
  return (
    <div className="min-h-screen bg-white font-sans">
      ...
      <header className="bg-white border-b border-gray-100 sticky top-0 z-40 shadow-sm">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <div>
              <span className="font-bold text-gray-900 text-lg">BaytFix</span>
            </div>
          </div>
      ...
      <section className="relative overflow-hidden bg-gradient-to-br from-emerald-700 to-emerald-500">
        ...
        <div className="relative max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-16 text-center">
          ...
          <h1 className="text-4xl sm:text-5xl font-extrabold text-white mb-3 leading-tight">
            Electrical & Plumbing<br />for your home
          </h1>
          ...
          <p className="text-white/70 text-base">كهرباء وسباكة وصيانة للمنازل</p>
        </div>
      </section>
      ...
      {catalogError && (
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          <div className="rounded-2xl border border-amber-200 bg-amber-50 p-5 text-amber-900" role="alert">
            <p className="font-semibold">Salla catalog unavailable</p>
            <p className="mt-1 text-sm">{catalogError} This menu does not use a local catalog fallback.</p>
          </div>
        </div>
      )}
      ...
      {/* Products */}
      {[
        { key: 'electrical', items: electrical, title: 'Electrical', titleAr: 'الكهرباء' },
        { key: 'plumbing',     items: plumbing,     title: 'Plumbing',     titleAr: 'السباكة' },
        { key: 'inspection',      items: inspection,      title: 'Inspections', titleAr: 'الفحص والصيانة' },
      ].map(section => (
              ...
              return (
                <div key={product.id} className="bg-white rounded-xl border border-gray-100 shadow-sm hover:shadow-md transition-all group overflow-hidden">
                  <div className="relative overflow-hidden h-40">
                    ...
                    <div className={`absolute top-2 right-2 flex items-center gap-1 text-xs px-1.5 py-0.5 rounded-full ${product.stock > 30 ? 'bg-emerald-100 text-emerald-700' : product.stock > 15 ? 'bg-amber-100 text-amber-700' : 'bg-red-100 text-red-700'}`}>
                      {product.stock > 30 ? 'Available' : product.stock > 15 ? 'Limited' : 'Low'}
                    </div>
                  </div>
                  <div className="p-3">
      ...
      <footer className="text-center py-6 text-gray-400 text-sm border-t border-gray-100">
        © 2026 BaytFix — كهرباء وسباكة للمنازل
      </footer>
    </div>
  );
}
