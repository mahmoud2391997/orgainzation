...
export function isLocationMessage(text: string) {
  ...
  const productWords = /زيارة|فيش|حنفية|تسريب|قاطع|سباكة|كهرباء|صيانة|فحص|outlet|faucet|leak|plumb|electric/i;
  ...
  if (isStatusQuery(input.text) && !parseCatalogOrder(input.text, await loadCatalog())) {
    ...
    if (parsed) {
      ...
      reply = catalog.length
        ? `أهلاً بك 🔧\nاطلب بكتابة الخدمة والعدد، مثل: "2 إصلاح فيش و 1 إصلاح حنفية"\nأو تصفح قائمة المتجر من سلة:\n${menuUrl}`
