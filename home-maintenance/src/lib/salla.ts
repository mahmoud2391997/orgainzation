...
export interface SallaCatalogProduct {
  ...
  name: string;
  nameAr: string;
  category: 'electrical' | 'plumbing' | 'inspection';
  unit: string;
  price: number;
  stock: number;
  imageUrl: string;
...
function catalogCategory(item: Record<string, any>): SallaCatalogProduct['category'] {
  ...
  if (text.includes('plumb') || text.includes('سباك') || text.includes('حنف') || text.includes('تسريب')) return 'plumbing';
  if (text.includes('inspect') || text.includes('صيان') || text.includes('فحص')) return 'inspection';
  return 'electrical';
}
