...
const CATEGORY_KEYWORDS: Record<string, string[]> = {
  electrical: ['كهرباء', 'كهربائي', 'electrical', 'electric', 'wiring', 'outlet', 'فيش', 'قاطع'],
  plumbing: ['سباكة', 'سباك', 'plumbing', 'plumb', 'pipe', 'faucet', 'leak', 'حنفية', 'تسريب', 'مواسير'],
  inspection: ['فحص', 'صيانة', 'inspection', 'maintenance', 'checkup', 'زيارة'],
};

const CATEGORY_NAMES: Record<string, { ar: string; en: string }> = {
  electrical: { ar: 'كهرباء', en: 'electrical' },
  plumbing: { ar: 'سباكة', en: 'plumbing' },
  inspection: { ar: 'فحص وصيانة', en: 'inspection' },
};
...
const SERVICE_KEYWORDS = [
  'electrical', 'electric', 'wiring', 'outlet', 'breaker', 'plumbing', 'plumb', 'pipe', 'faucet',
  'leak', 'drain', 'inspection', 'maintenance', 'handyman', 'home service', 'home repair',
  'كهرباء', 'كهربائي', 'فيش', 'قاطع', 'سباكة', 'سباك', 'حنفية', 'تسريب', 'مواسير', 'مجاري',
  'فحص', 'صيانة', 'صيانة منزل', 'خدمات منزلية',
];
...
export function isServiceCatalogItem(item: SallaRecord): boolean {
...
export function inferCategories(categoryLabelText: string): [string, string] {
  ...
  if (label.includes('plumb') || label.includes('سباك') || label.includes('حنف') || label.includes('تسريب') || label.includes('ماسورة') || label.includes('مواسير')) {
    return ['plumbing', 'visit'];
  }
  if (label.includes('inspect') || label.includes('maintenance') || label.includes('فحص') || label.includes('صيانة')) {
    return ['inspection', 'visit'];
  }
  return ['electrical', 'visit'];
}
...
export async function syncSallaProduct(payload: any, auth = undefined) {
  ...
  return prisma.product.create({ data: { ...data, sallaProductId: id, shopPrice: 0, wholesalePrice: 0, category: 'electrical', unit: 'visit', stock: Number(item.quantity ?? 0), imageUrl } });
}
...
export async function pushCatalogToSalla() {
  ...
  for (const product of products) {
    const localCategory = product.category || 'electrical';
