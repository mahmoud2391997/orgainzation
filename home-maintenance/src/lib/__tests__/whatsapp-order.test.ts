...
const catalog = [
  { id: '11', name: 'Outlet Repair', nameAr: 'إصلاح فيش', category: 'electrical', unit: 'visit', price: 15, stock: 40, imageUrl: '', purchasable: true },
];
...
describe('intent helpers', () => {
  it('detects greetings as menu requests and status questions separately', () => {
    ...
    expect(isStatusQuery('2 إصلاح فيش')).toBe(false);
  });
});
...
describe('handleWhatsAppText', () => {
  ...
  it('parses a typed order against the Salla catalog', async () => {
    const result = await handleWhatsAppText({ phone: '966500000000', text: '2 إصلاح فيش' });
    expect(result.reply).toContain('إصلاح فيش');
    expect(result.reply).toContain('30.00');
  ...
  it('places the draft on Salla after address + confirmation', async () => {
    ...
    mocks.message.findFirst.mockResolvedValue({
      text: JSON.stringify({ items: [{ id: '11', name: 'Outlet Repair', nameAr: 'إصلاح فيش', qty: 2, unit: 'visit', price: 15, stock: 40, purchasable: true }], unmatched: [], total: 30, location: 'الرياض حي النخيل' }),
    });
