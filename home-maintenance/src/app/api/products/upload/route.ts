...
export async function POST(req: Request) {
  ...
  const validCategories = ['electrical', 'plumbing', 'inspection'];
  const products = rows
    .map(r => ({
      name: String(r.name || '').trim(),
      nameAr: String(r.name_ar || '').trim(),
      category: validCategories.includes(String(r.category)) ? String(r.category) : 'electrical',
      unit: String(r.unit || 'visit').trim(),
