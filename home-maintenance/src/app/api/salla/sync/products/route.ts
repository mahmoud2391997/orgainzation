...
import { isServiceCatalogItem, matchesLocalCatalogName, categoryLabel, inferCategories } from '@/lib/salla-sync';
  ...
  try {
    ...
    while (hasMore && page <= 100) {
      ...
      for (const item of result.data ?? []) {
        ...
        if (!isServiceCatalogItem(item as Record<string, unknown>) && !matchesLocalCatalogName(name, catalogNames)) {
          skipped++;
          continue;
        }
        accepted.push(id);
