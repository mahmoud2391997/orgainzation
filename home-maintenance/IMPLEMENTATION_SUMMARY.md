# BaytFix — WhatsApp + Salla Home Services

Identical ordering stack (Next.js, Prisma, Salla, WhatsApp Cloud API) retargeted to **electrical and plumbing home maintenance**.
...
- **16 Services Created**: 8 electrical, 5 plumbing, 3 inspection/maintenance
- **Tiered Pricing**: Homeowner, building, and contractor rates for each job
- **Stock Levels**: Visit slots treated as inventory
- **Bilingual Names**: English and Arabic service names
...
### Scenario: Building manager books jobs

1. **WhatsApp Message**: "أحتاج إصلاح فيش، إصلاح حنفية، وتسليك مجاري"
2. **Bot Parsing**: Gemini extracts jobs and quantities
3. **Menu Page Creation**: 
 - Menu page ID: `uuid-abc123`
 - Customer ID linked to phone: +20 555 111 2222
4. **WhatsApp Link Sent**: "اختر الخدمات: https://app.com/menu/uuid-abc123"
5. **Customer Reviews**: Opens menu, sees items pre-filled
6. **Checkout**: Reviews prices (building/contractor pricing applied automatically)
7. **Order Created**: Status = "pending"
8. **Dashboard Alert**: Business sees new job from "Al Noor Compound"
9. **Confirmation**: Business clicks "Confirm" button
10. **WhatsApp Notification**: Template message sent with job ID
