...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    // ── Flow-based rule engine (fast path before calling Mistral) ──
    ...
    if (messageCount === 2) {
      ...
      if (RETAIL_PATTERNS.test(message)) {
        ...
        detectedType = "shop";
        reply = "رائع! ستحصل على أسعار العمارات وإدارة العقارات. أرسل لنا موقعك للتوصيل.";
      } else if (RESTAURANT_PATTERNS.test(message)) {
        detectedType = "restaurant";
        reply = "ممتاز! ستحصل على أسعار المقاولين. أرسل لنا موقعك.";
      } else {
    ...
    // ── Call Mistral AI for NLP order parsing ──
    ...
    const prompt = `You are an AI assistant for a home electrical and plumbing WhatsApp ordering system.
...
Respond ONLY with valid JSON (no markdown):
{
  ...
  "orderData": {
    "items": [{"name": "Outlet Repaires", "qty": 5, "unit": "kg", "price": 8}],
    "total": 40,
    "location": "Riyadh"
  }
}
If no order, set "orderData": null.`;
      ...
      "https://api.mistral.ai/v1/chat/completions",
      {
        ...
        body: JSON.stringify({
          ...
          messages: [
            {
              role: "system",
              content: "You are an AI assistant for a home electrical and plumbing WhatsApp ordering system. Always respond with valid JSON only, no markdown formatting."
            },
            {
              role: "user",
              content: prompt
    ...
    if (!mistralRes.ok) {
      ...
      await supabase.from("system_logs").insert({
        level: "error",
        service: "mistral-parse",
        message: `Mistral API error: ${mistralRes.status}`,
