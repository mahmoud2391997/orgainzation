...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const templates: Record<string, { subject: string; html: string }> = {
      order_confirmed: {
        subject: `تأكيد الطلب ${orderId ?? ""} - BaytFix`,
      ...
      custom: {
        subject: subject ?? "BaytFix Notification",
    ...
    const emailPayload = {
      ...
      from: { email: FROM_EMAIL, name: "BaytFix" },
...
function buildOrderEmail(order: Record<string, unknown> | null, frontendUrl: string): string {
  ...
  <div style="font-family: 'Segoe UI', sans-serif; max-width: 600px; margin: 0 auto; padding: 20px;">
    <div style="background: linear-gradient(135deg, #059669, #10b981); padding: 30px; border-radius: 12px 12px 0 0; text-align: center;">
      <h1 style="color: white; margin: 0; font-size: 24px;">BaytFix 🔧</h1>
