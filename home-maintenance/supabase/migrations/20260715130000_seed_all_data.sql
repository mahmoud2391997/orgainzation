/*
# Seed All Data for BaytFix WhatsApp Ordering System
...
SELECT
  ...
  'customer',
  'مرحبا، أريد إصلاح فيش وإصلاح حنفية',
  '09:30 AM',
  'order_request'
...
SELECT
  ...
  'customer',
  'السلام عليكم، نحتاج صيانة دورية لكهرباء وسباكة العمارة',
  '10:15 AM',
  'inquiry'
...
SELECT
  (SELECT id FROM conversations WHERE phone = '+20 987 654 3210' LIMIT 1),
  'bot',
  'أهلا! نحن نوفر أسعار خاصة للعمارات. يرجى زيارة قائمتك الخاصة',
  '10:16 AM',
  'greeting'
WHERE EXISTS (SELECT 1 FROM conversations WHERE phone = '+20 987 654 3210')
...
SELECT
  ...
  'customer',
  'نحتاج صيانة كهرباء وسباكة لعدة وحدات',
  '11:00 AM',
  'inquiry'
...
SELECT
  (SELECT id FROM conversations WHERE phone = '+20 555 111 2222' LIMIT 1),
  'bot',
  'مرحبا! نحن نوفر أسعار خاصة للمقاولين. إليك قائمة الأسعار الخاصة بك',
  '11:01 AM',
  'menu_link'
WHERE EXISTS (SELECT 1 FROM conversations WHERE phone = '+20 555 111 2222');
...
VALUES
  -- Order 1: 2kg Outlet Repair (retail), 3 Lettuce (retail)
  ('ORD-001', 'Outlet Repair', 2, 'kg', 2.50),
  ('ORD-001', 'Lettuce', 3, 'piece', 1.50),
  ('ORD-001', 'Faucet Repair', 2, 'kg', 1.80),
  ...
  ('ORD-003', 'Outlet Repair', 20, 'kg', 2.00),
  ('ORD-003', 'Onion', 15, 'kg', 0.90),
  ('ORD-003', 'Bell Pepper', 10, 'kg', 2.80),
  ('ORD-003', 'Lettuce', 50, 'piece', 1.20),

  -- Order 4: Contractor order (wholesale) - mixed electrical & plumbing
  ('ORD-004', 'Outlet Repair', 50, 'kg', 1.50),
