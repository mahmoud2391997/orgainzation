        ...
        <div className="shell grid-2">
          {services.map((service) => (
            <article className="surface service-card" key={service.id}>
              ...
              <div className="card-body"><div className="card-icon"><Icon name={service.icon} size={19} /></div><h2 className="h3">{service.name}</h2><p>{service.detailedDescription}</p><div className="tag-row">{service.technologies.map((technology) => <span className="tag" key={technology}>{technology}</span>)}</div><div className="card-footer"><Link className="card-link" href={`/services/${service.slug}`}>Explore this capability <ArrowRight size={14} /></Link></div></div>
            </article>
          ))}
        </div>
      </section>
